...
/** "a of b" as a percentage — "94%" — or an en dash when b is zero. */
...
/** A local calendar date as `YYYY-MM-DD`, the value a date input holds. */
export function isoDay(ts: number | Date = Date.now()): string {
  const d = new Date(ts);
  return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}-${String(d.getDate()).padStart(2, "0")}`;
}

/** The `YYYY-MM-DD` date `days` before today. */
