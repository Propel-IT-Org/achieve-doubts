...
export const questions = pgTable(
	...
	(table) => [
		...
		index("idx_questions_taxonomy").on(table.subjectId, table.bookId, table.chapterId),
...
export const threadMessages = pgTable(
	...
	(table) => [index("idx_thread_question").on(table.questionId, table.createdAt)],
);
...
export const comments = pgTable(
	...
	(table) => [index("idx_comments_question").on(table.questionId, table.createdAt)],
);
...
export const solutionsRelations = relations(solutions, ({ one }) => ({
	question: one(questions, { fields: [solutions.questionId], references: [questions.id] }),
...
export const threadMessagesRelations = relations(threadMessages, ({ one }) => ({
	...
	author: one(user, { fields: [threadMessages.authorId], references: [user.id] }),
}));
